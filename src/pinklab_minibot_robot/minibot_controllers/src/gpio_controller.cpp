...
namespace minibot_controllers
{
    ...
    {
        enable_motor_state_ = static_cast<bool>(state_interfaces_[StateInterfaces::ENABLE_MOTOR_STATE].get_optional().value_or(0.0));
        left_lamp_state_ = static_cast<uint8_t>(state_interfaces_[StateInterfaces::LEFT_LAMP_STATE].get_optional().value_or(0.0));
        right_lamp_state_ = static_cast<uint8_t>(state_interfaces_[StateInterfaces::RIGHT_LAMP_STATE].get_optional().value_or(0.0));
        range_sensor_state_ = static_cast<uint16_t>(state_interfaces_[StateInterfaces::RANGE_SENSOR_STATE].get_optional().value_or(0.0));
    ...
    {
        if (!command_interfaces_[CommandInterfaces::ENABLE_MOTOR_CMD].set_value(msg.data ? 1.0 : 0.0)) {
            RCLCPP_WARN(get_node()->get_logger(), "Failed to set enable motor command");
        }
    }

    void GPIOController::callback_lamp_command(const minibot_interfaces::msg::LampCommand & msg)
    {
        if (!command_interfaces_[CommandInterfaces::LEFT_LAMP_CMD].set_value((double)msg.l_command)) {
            RCLCPP_WARN(get_node()->get_logger(), "Failed to set left lamp command");
        }
        if (!command_interfaces_[CommandInterfaces::RIGHT_LAMP_CMD].set_value((double)msg.r_command)) {
            RCLCPP_WARN(get_node()->get_logger(), "Failed to set right lamp command");
        }
    }
}
